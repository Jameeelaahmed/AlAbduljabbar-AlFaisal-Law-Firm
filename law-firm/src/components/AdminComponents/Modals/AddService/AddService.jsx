                ...
                >
                    {({ isSubmitting, errors, touched }) => (
                        ...
                        >
                            ...
                            {/* Category Select */}
                            <div className="space-y-2">
                                <label htmlFor="categoryId" className="flex items-center gap-2 text-sm font-semibold text-primary">
                                    {t("Services.Category")}
                                </label>
                                <Field name="categoryId" as="select">
                                    {({ field }) => (
                                        <select
                                            ...
                                            className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 ${errors.categoryId && touched.categoryId ? "border-red-300 focus:ring-red-200 bg-red-50" : "border-gray-300 focus:ring-primary focus:border-primary"
                                                }`}
                                            disabled={isSubmitting || categoriesLoading}
                                        >
                                            <option value="">{t("Services.Select a category")}</option>
                                            {categories?.map((c) => (
                                                <option key={c.id} value={c.id}>
                                                    {c.nameEn || c.name || `${c.id}`}
                                                </option>
                                            ))}
                                        </select>
                                    )}
                                </Field>
                                <ErrorMessage name="categoryId" component="div" className="text-red-500 text-xs mt-1 font-medium" />
                            </div>
